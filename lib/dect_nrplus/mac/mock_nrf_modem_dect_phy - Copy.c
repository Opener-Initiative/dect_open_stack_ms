...
{
	printk("[MOCK_PHY] Performing complete reset of context %p\n", (void*)ctx);
...
{
	printk("[MOCK_PHY] Initializing context %p (Role: %s)\n", (void*)ctx, (mac_ctx->role == MAC_ROLE_FT ? "FT" : "PT"));
...
{
	if (!dest_ctx || !packet) return -EINVAL;
	...
	for (size_t i = 0; i < num_contexts; i++) {
		...
		for (int j = 0; j < MOCK_TIMELINE_MAX_EVENTS; j++) {
			...
			if (ctx->timeline[j].active) {
				printk("[MOCK_SIM_DBG] Timeline Op[%d] (%u) active. End: %llu us\n", 
					j, ctx->timeline[j].handle, ctx->timeline[j].end_time_us);
			}
		...
		for (int j = 0; j < MOCK_RX_QUEUE_MAX_PACKETS; j++) {
			...
			if (ctx->rx_queue[j].active) {
				printk("[MOCK_SIM_DBG] RX Queue Pkt[%d] active. Arrival: %llu us\n", 
					j, ctx->rx_queue[j].reception_time_us);
			}
			if (ctx->rx_queue[j].active && ctx->rx_queue[j].reception_time_us > current_time) {
...
{
	printk("[MOCK_PROCESS_EVENTS] Start (Time: %llu, Role: %s)\n", current_time_us, ctx->mac_ctx->role == MAC_ROLE_FT ? "FT" : "PT");

	/* --- PHASE 1: Start operations --- */
	for (int i = 0; i < MOCK_TIMELINE_MAX_EVENTS; ++i) {
		mock_scheduled_operation_t *op = &ctx->timeline[i];
		if (op->active && !op->running && op->start_time_us <= current_time_us) {
			printk("[MOCK_OP_START] Handle %u now RUNNING at time %llu\n", op->handle, current_time_us);
			op->running = true;
		}
	}

	/* --- PHASE 2: Process RX Queue --- */
	int pkt_processed_in_this_run;
	do {
		pkt_processed_in_this_run = 0;
		int next_pkt_idx = -1;
		...
		if (next_pkt_idx != -1) {
			...
			pkt->pcc_data.transaction_id = ctx->transaction_id_counter;

			for (int j = 0; j < MOCK_TIMELINE_MAX_EVENTS; ++j) {
				mock_scheduled_operation_t *rx_op = &ctx->timeline[j];
				
				if (rx_op->active && rx_op->running && rx_op->type == MOCK_OP_TYPE_RX && pkt->carrier == rx_op->carrier) {
					mock_phy_set_active_context(ctx);

					if (collision) {
						printk("[MOCK_RF_COLLISION] Collision on Carrier %u at %llu. Sending PCC_ERROR.\n", pkt->carrier, pkt->reception_time_us);
						struct nrf_modem_dect_phy_event pcc_err = {
							.id = NRF_MODEM_DECT_PHY_EVT_PCC_ERROR,
							.time = pkt->reception_time_us,
							.pcc_crc_err = { .handle = rx_op->handle, .transaction_id = pkt->pcc_data.transaction_id }
						};
						mock_phy_send_event(&pcc_err);
					} else {
						printk("[MOCK_RX_DELIVERY] Delivering packet (time %llu) to Carrier %u, Handle %u\n", pkt->reception_time_us, pkt->carrier, rx_op->handle);
						
						/* RESTORED DATA DEBUG */
						printk("[MOCK_RX_DELIVERY] Payload (8 bytes): ");
						for (int k=0; k<8 && k<pkt->pdc_len; k++) printk("%02x ", pkt->pdc_payload[k]);
						printk("\n");
						...
						uint64_t pdc_event_time_us = pkt->reception_time_us + 50; 
						struct nrf_modem_dect_phy_event pdc_evt = {
							.id = NRF_MODEM_DECT_PHY_EVT_PDC,
							.time = pdc_event_time_us,
							.pdc = { .handle = rx_op->handle, .transaction_id = pkt->pcc_data.transaction_id, .data = pkt->pdc_payload, .len = pkt->pdc_len }
						};
			...
			if (!rx_op_found) {
				printk("[MOCK_FAIL] No matching RX op found for packet at time %llu on Carrier %u.\n", pkt->reception_time_us, pkt->carrier);
			}
			pkt->active = false;
			pkt_processed_in_this_run = 1;
		}
	} while (pkt_processed_in_this_run);

	/* --- PHASE 3: Complete operations --- */
	for (int i = 0; i < MOCK_TIMELINE_MAX_EVENTS; ++i) {
		mock_scheduled_operation_t *op = &ctx->timeline[i];
		if (op->active && op->running && op->end_time_us <= current_time_us) {
			printk("[MOCK_PROCESS_DBG] Completing op in slot %d (Handle %u) EndTime: %llu\n", i, op->handle, op->end_time_us);
			mock_phy_set_active_context(ctx);
			if (op->type == MOCK_OP_TYPE_RSSI) {
				...
				struct nrf_modem_dect_phy_event rssi_evt = {
					...
					.time = op->end_time_us,
					.rssi = { .handle = op->handle, .meas_start_time = op->start_time_us, .carrier = op->carrier, .meas_len = 1, .meas = dummy_meas }
				};
...
/* ========================================================================
 * NORDIC DECT PHY API IMPLEMENTATION
 * ======================================================================== */
...
int nrf_modem_dect_phy_init(void)
{
	if (g_active_phy_ctx) {
		...
		struct nrf_modem_dect_phy_event event = { .id = NRF_MODEM_DECT_PHY_EVT_INIT, .init = {.err = NRF_MODEM_DECT_PHY_SUCCESS}};
...
{
	if (g_force_phy_schedule_failure) return g_phy_schedule_failure_code;
	if (g_active_phy_ctx->state != PHY_STATE_ACTIVE) return -1;
	
	int slot = find_free_timeline_slot(g_active_phy_ctx);
	if (slot < 0) return -ENOMEM;

	uint64_t start_time_us = (params->start_time == 0) ? MAX(k_ticks_to_us_floor64(k_uptime_ticks()), get_timeline_end_time(g_active_phy_ctx)) : params->start_time;
	uint64_t duration_us = modem_ticks_to_us(params->data_size * 8 * 10, NRF_MODEM_DECT_MODEM_TIME_TICK_RATE_KHZ);
	if (duration_us < 100) duration_us = 100;

	g_active_phy_ctx->timeline[slot] = (mock_scheduled_operation_t){
		.active = true, .running = false, .handle = params->handle, .type = MOCK_OP_TYPE_TX,
		.start_time_us = start_time_us, .end_time_us = start_time_us + duration_us,
		.pdu_len = params->data_size, .carrier = params->carrier, .context = g_active_phy_ctx->mac_ctx,
	};
	if (params->data) {
	...
	printk("  - Transmitter: %s | Carrier: %u | Handle: %u | Start: %llu us\n", 
		   (g_active_phy_ctx->mac_ctx->role == MAC_ROLE_PT ? "PT" : "FT"), params->carrier, params->handle, start_time_us);

	/* RESTORED TX DEBUG */
	if (params->data) {
		printk("  - TX Data (8 bytes): ");
		for (int k=0; k<8 && k<params->data_size; k++) printk("%02x ", params->data[k]);
		printk("\n");
	}
	...
	for (size_t i = 0; i < g_active_phy_ctx->num_peers; i++) {
		...
		if (peer_ctx) {
			mock_rx_packet_t rx_pkt = {
				.active = true, .reception_time_us = start_time_us + 100, .duration_us = duration_us, .carrier = params->carrier,
				.pcc_data = { .phy_type = params->phy_type, .hdr = *params->phy_header, .rssi_2 = -60, .snr = 80 },
				.pdc_len = params->data_size,
			};
			if (params->data) memcpy(rx_pkt.pdc_payload, params->data, params->data_size);
			printk("[MOCK_TX_DBG]   -> Queuing packet for peer context %p\n", (void *)peer_ctx);
...
int nrf_modem_dect_phy_rx(const struct nrf_modem_dect_phy_rx_params *params)
{
	...
	uint64_t start_time_us = (params->start_time == 0) ? MAX(k_ticks_to_us_floor64(k_uptime_ticks()), get_timeline_end_time(g_active_phy_ctx)) : params->start_time;
	uint64_t end_time_us = (params->duration == 0 || params->duration == 0xFFFFFFFF) ? UINT64_MAX : (start_time_us + modem_ticks_to_us(params->duration, NRF_MODEM_DECT_MODEM_TIME_TICK_RATE_KHZ));

	g_active_phy_ctx->timeline[slot] = (mock_scheduled_operation_t){
		.active = true, .handle = params->handle, .type = MOCK_OP_TYPE_RX,
		.start_time_us = start_time_us, .end_time_us = end_time_us, .carrier = params->carrier,
	};
	...
	printk("[MOCK_PHY] RX scheduled. Handle: %u, Carrier: %u, Start: %llu us, End: %llu\n", params->handle, params->carrier, start_time_us, end_time_us);
	return 0;
}
...
int nrf_modem_dect_phy_rssi(const struct nrf_modem_dect_phy_rssi_params *params)
{
	...
	g_active_phy_ctx->timeline[slot] = (mock_scheduled_operation_t){
		.active = true, .handle = params->handle, .type = MOCK_OP_TYPE_RSSI,
		.start_time_us = start_time_us, .end_time_us = start_time_us + duration_us, .carrier = params->carrier,
	};
	printk("[MOCK_PHY] RSSI scheduled. Handle: %u, Carrier: %u, Start: %llu us\n", params->handle, params->carrier, start_time_us);
	return 0;
}
...
int nrf_modem_dect_phy_activate(enum nrf_modem_dect_phy_radio_mode mode)
{
	...
	struct nrf_modem_dect_phy_event event = { .id = NRF_MODEM_DECT_PHY_EVT_ACTIVATE, .activate = {.err = NRF_MODEM_DECT_PHY_SUCCESS}};
	mock_phy_send_event(&event);
	printk("[MOCK_PHY] PHY Activated.\n");
	return 0;
}
...
int nrf_modem_dect_phy_cancel(uint32_t handle)
{
	...
	bool found = false;
	for (int i = 0; i < MOCK_TIMELINE_MAX_EVENTS; ++i) {
		if (g_active_phy_ctx->timeline[i].active && g_active_phy_ctx->timeline[i].handle == handle) {
			g_active_phy_ctx->timeline[i].active = false;
			found = true;
			struct nrf_modem_dect_phy_event op_event = { .id = NRF_MODEM_DECT_PHY_EVT_COMPLETED, .time = k_ticks_to_us_floor64(k_uptime_ticks()), .op_complete = {.handle = handle, .err = NRF_MODEM_DECT_PHY_ERR_OP_CANCELED}};
			mock_phy_set_active_context(g_active_phy_ctx);
			mock_phy_send_event(&op_event);
			break;
		}
	}
	struct nrf_modem_dect_phy_event cancel_event = { .id = NRF_MODEM_DECT_PHY_EVT_CANCELED, .time = k_ticks_to_us_floor64(k_uptime_ticks()), .cancel = {.handle = handle, .err = found ? NRF_MODEM_DECT_PHY_SUCCESS : NRF_MODEM_DECT_PHY_ERR_NOT_FOUND}};
	mock_phy_set_active_context(g_active_phy_ctx);
	mock_phy_send_event(&cancel_event);
	printk("[MOCK_PHY_CANCEL] Handle: %u, Found: %d\n", handle, found);
	return 0;
}

int nrf_modem_dect_phy_latency_get(void)
{
	static struct nrf_modem_dect_phy_latency_info mock_latency;
	memset(&mock_latency, 0, sizeof(mock_latency));
	mock_latency.operation.receive.idle_to_active = 6912;
	mock_latency.operation.transmit.idle_to_active = 6912;
	struct nrf_modem_dect_phy_event event = { .id = NRF_MODEM_DECT_PHY_EVT_LATENCY, .time = k_ticks_to_us_floor64(k_uptime_ticks()), .latency_get = { .err = NRF_MODEM_DECT_PHY_SUCCESS, .latency_info = &mock_latency }};
	mock_phy_send_event(&event);
	return 0;
}

/* --- Internal Helpers --- */
static void mock_phy_send_event(const struct nrf_modem_dect_phy_event *event) { if (g_event_handler) g_event_handler(event); }

static int find_free_timeline_slot(mock_phy_context_t *ctx) {
	printk("[MOCK_HELPER] Searching for free timeline slot...\n");
	for (int i = 0; i < MOCK_TIMELINE_MAX_EVENTS; ++i) { 
		if (!ctx->timeline[i].active) {
			printk("[MOCK_HELPER] Found free slot: %d\n", i);
			return i; 
		}
	}
	printk("[MOCK_HELPER] ERROR: No free timeline slot!\n");
	return -1;
}
...
static uint64_t get_timeline_end_time(mock_phy_context_t *ctx) {
	...
	printk("[MOCK_HELPER] Latest timeline end time: %llu us\n", latest_end);
	return latest_end;
}

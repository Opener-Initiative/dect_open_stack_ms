...
//         dect_mac_context_t *ctx = dect_mac_get_active_context(); // Get context again for safety
...
// 		return -ENOMEM;
// 	}
// 	full_mac_pdu_phy_buf_slab_alloc = pdu_sdu->data;
...
//     /* The caller is responsible for freeing the buffer on error. Do nothing here. */
//     // dect_mac_buffer_free((mac_sdu_t *)pdu_sdu);
...
if (ctx->state > 2 && ctx->state != 17 ){
	...
	// 		LOG_INF("DP_SVC_TX: Attempting re-TX for HARQ proc %d", i);
	// 		int peer_idx = (ctx->role == MAC_ROLE_FT)
	// 				       ? dect_mac_core_get_peer_slot_idx(
	// 						 harq_p->peer_short_id_for_ft_dl)
	// 				       : 0;
	...
	// 				? ft_get_next_tx_opportunity(peer_idx, &start_time,
	// 							     &carrier, &schedule)
	// 				: pt_get_next_tx_opportunity(&start_time, &carrier,
	// 							     &schedule);

	// 		if (opportunity_found) {
	// 			send_data_mac_sdu_via_phy_internal(
	...
	// 		LOG_DBG("DP_SVC_TX: Cleaning up completed HARQ process %d.", i);
	// 		dect_mac_buffer_free(harq_p->sdu);
	// 		harq_p->sdu = NULL;
	// 		harq_p->is_active = false;
	// 	}
...
// #endif
	...
	if (ctx->role == MAC_ROLE_FT) {
		...
		// if (opportunity_found) {        
		// // if (pt_get_next_tx_opportunity(&start_time, &carrier, &schedule)) {
		// 	for (int j = 0; j < MAC_FLOW_COUNT; j++) {
        //         printk("    - Checking queue for Flow %d. Is empty: %s\n", j, sys_dlist_is_empty(mac_tx_dlists[j]) ? "yes" : "no");
...
// bool opportunity_found = pt_get_next_tx_opportunity(&start_time, &carrier, &schedule);
		...
		// 	/* Special case for sending the release message */
		// 	if (pt_get_next_tx_opportunity(&start_time, &carrier, &schedule)) {
		// 		/* The release message is not in a queue, it's a direct action */
		// 		/* We need a way to trigger the send action from here.
		// 		 * The best way is to have a flag in the context.
		// 		 */
		// 		if (ctx->role_ctx.pt.release_pending) {
		// 			dect_mac_sm_pt_send_association_release_action(&ctx->role_ctx.pt.associated_ft);
		// 			ctx->role_ctx.pt.release_pending = false;
		// 		}
		// 	}
		// } else {
		// 	bool opportunity_found = pt_get_next_tx_opportunity(&start_time, &carrier, &schedule);
		...
		// 	if (pt_get_next_tx_opportunity(&start_time, &carrier, &schedule)) {
		// 		/* The release message is not in a queue, it's a direct action */
        //         printk("******************Special case for sending the release message*******************  \n");
...
// // printk(" IE_TYPE_USER_DATA_FLOW_1            0b000011\n");
// // printk(" IE_TYPE_USER_DATA_FLOW_2            0b000100\n");
// // printk(" IE_TYPE_USER_DATA_FLOW_3            0b000101\n");
// // printk(" IE_TYPE_USER_DATA_FLOW_4            0b000110\n");     
// printk("RX_SDU_HANDLER:  ie_type_from_mux %d >= %d:IE_TYPE_USER_DATA_FLOW_1 && <= %d:IE_TYPE_USER_DATA_FLOW_1 \n", ie_type_from_mux, (int)IE_TYPE_USER_DATA_FLOW_1, (int)IE_TYPE_USER_DATA_FLOW_4);

//         // TODO: fix the logic as the comparision is not working
// 		// if (ie_type_from_mux >= (int)IE_TYPE_USER_DATA_FLOW_1 &&
// 		//     ie_type_from_mux <= (int)IE_TYPE_USER_DATA_FLOW_4) {
// 		if (ie_type_from_mux) {        		
...
//                         printk("RX_SDU_HANDLER: Appending SDU to dlist - IE type: 0x%X, length: %u, data ptr: %p, node ptr: %p\n",
				      ...
				      uint32_t transmitter_long_rd_id)
{
	...
	while (remaining_sdu_area_len > 0) {
		...
		//     ie_type_from_mux <= (int)IE_TYPE_USER_DATA_FLOW_4) {
		if (ie_type_from_mux) {        
            printk("RX_SDU_HANDLER: User data flow detected, checking DLC queue...\n");

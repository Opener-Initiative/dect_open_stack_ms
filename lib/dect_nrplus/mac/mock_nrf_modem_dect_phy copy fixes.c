                ...
                if (rx_op->active && rx_op->running && rx_op->type == MOCK_OP_TYPE_RX) {
                    if (pkt->carrier == rx_op->carrier) {
                        ...
                        if (collision) {
                            ...
                            /* PCC is reported at the start of the packet reception */
                            ...
                            /* PDC (Payload Decode Complete) occurs at the end */
                            struct nrf_modem_dect_phy_event pdc_evt = {
                                .id = NRF_MODEM_DECT_PHY_EVT_PDC,
                                .time = pkt->reception_time_us + pkt->duration_us,
...
int nrf_modem_dect_phy_tx(const struct nrf_modem_dect_phy_tx_params *params)
{
    ...
    for (size_t i = 0; i < g_active_phy_ctx->num_peers; i++) {
        ...
        if (peer_ctx) {
            ...
            /* Packet is in the air. Time of reception starts after transmission
             * finishes (start_time + duration). Plus a small propagation delay. */
            uint64_t reception_time = start_time_us + duration_us + 10;
            
            mock_rx_packet_t rx_pkt = {
                .active = true,
                .reception_time_us = reception_time,
...
{ 
    struct nrf_modem_dect_phy_event time_event = {
        .id = NRF_MODEM_DECT_PHY_EVT_TIME,
        .time = k_ticks_to_us_floor64(k_uptime_ticks()),
        .time_get = {.time = k_ticks_to_us_floor64(k_uptime_ticks())}};
    mock_phy_send_event(&time_event);
    return 0; 
}
...
static void mock_phy_send_event(const struct nrf_modem_dect_phy_event *event)
{
    if (g_event_handler) {
        ...
        struct nrf_modem_dect_phy_event event_to_send = *event;

        // Convert to milliseconds timestamps
        event_to_send.time = modem_us_to_ticks(event->time, NRF_MODEM_DECT_MODEM_TIME_TICK_RATE_KHZ);

        g_event_handler(&event_to_send);
    } else {

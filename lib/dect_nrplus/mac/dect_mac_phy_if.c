...
static void phy_event_handler_callback(const struct nrf_modem_dect_phy_event *event)
{
	...
	if (is_op_event && event_ctx == NULL) { /* Only search map if override was not used */
		...
		for (int i = 0; i < MAX_CONCURRENT_PHY_OPS; i++) {
			// LOG_DBG("g_op_handle_map[%d].handle:%d == event_handle:%d ", i, g_op_handle_map[i].handle, event_handle);
			last = i;
		...
		// LOG_DBG("g_op_handle_map last:%d < %d", last, MAX_CONCURRENT_PHY_OPS);
	}
	k_spin_unlock(&g_phy_if_lock, key);

	if (!event_ctx) {
		LOG_DBG("[PHY_IF HANDLER] ***WARNING***: Could not find context for event %d (handle %u). Using globally active context.",
	...
	case NRF_MODEM_DECT_PHY_EVT_INIT:
		LOG_INF(" NRF_MODEM_DECT_PHY_EVT_INIT result: %d (Temp: %dC, Volt: %umV, Limit: %uC)",
		...
		if (event->init.err == NRF_MODEM_DECT_PHY_SUCCESS) {
			LOG_INF(" DECT PHY Initialized by modem. Requesting latency information.");
			err = nrf_modem_dect_phy_latency_get();
			if (err != 0) {
				LOG_ERR(" Failed to request nrf_modem_dect_phy_latency_get(): %d", err);
			}
		} else {
			LOG_ERR(" Modem DECT PHY Initialization failed with error %d.", event->init.err);
		}
	...
	case NRF_MODEM_DECT_PHY_EVT_LATENCY:
		LOG_INF(" NRF_MODEM_DECT_PHY_EVT_LATENCY result: %d", event->latency_get.err);
		if (event->latency_get.err == NRF_MODEM_DECT_PHY_SUCCESS && event->latency_get.latency_info) {
			...
			if (ctx) {
				...
				LOG_INF(" Latency info stored in MAC context (e.g., sched_startup_us: %u, idle_to_rx_us: %u).",
					ctx->phy_latency.scheduled_operation_startup_us, ctx->phy_latency.idle_to_active_rx_us);
			} else {
				LOG_ERR(" MAC context NULL, cannot store latency info.");
			}
		} else if (event->latency_get.err != NRF_MODEM_DECT_PHY_SUCCESS) {
			LOG_ERR(" Failed to get latency info from modem: %d", event->latency_get.err);
		} else {
			LOG_ERR(" Latency info event success, but latency_info pointer is NULL from modem.");
		}
		should_queue_msg = false;
		break;

	case NRF_MODEM_DECT_PHY_EVT_DEINIT:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_DEINIT result: %d", event->deinit.err);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_CONFIGURE:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_CONFIGURE result: %d", event->configure.err);
		should_queue_msg = false;
		k_sem_give(&phy_init_sem);
		break;
	case NRF_MODEM_DECT_PHY_EVT_RADIO_CONFIG:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_RADIO_CONFIG result: %d (handle %u)",
			event->radio_config.err, event->radio_config.handle);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_ACTIVATE:
		 LOG_INF("NRF_MODEM_DECT_PHY_EVT_ACTIVATE result: %d (Temp: %dC, Volt: %umV)",
			event->activate.err, event->activate.temp, event->activate.voltage);
		should_queue_msg = false;
		k_sem_give(&phy_init_sem);
		break;
	case NRF_MODEM_DECT_PHY_EVT_DEACTIVATE:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_DEACTIVATE result: %d", event->deactivate.err);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_CANCELED:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_CANCELED (for cancel op itself) result: %d for handle %u",
			event->cancel.err, event->cancel.handle);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_TIME:
		LOG_DBG("NRF_MODEM_DECT_PHY_EVT_TIME: current modem time is %llu", event->time);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_CAPABILITY:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_CAPABILITY result: %d. Capability data ptr: %p", event->capability_get.err, event->capability_get.capability);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_BANDS:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_BANDS result: %d. Band count: %u", event->band_get.err, event->band_get.band_count);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_LINK_CONFIG:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_LINK_CONFIG result: %d", event->link_config.err);
		should_queue_msg = false;
		break;
	case NRF_MODEM_DECT_PHY_EVT_STF_CONFIG:
		LOG_INF("NRF_MODEM_DECT_PHY_EVT_STF_CONFIG result: %d", event->stf_cover_seq_control.err);
		should_queue_msg = false;
		break;

	default:
		LOG_WRN("Received unknown/unhandled PHY event ID: 0x%02X from modem. Not queueing.", event->id);
		should_queue_msg = false;
		break;
	}

	if (should_queue_msg) {
		LOG_DBG("PRE-QUEUE CHECK: msg.type = %d (%s)\t", msg_to_queue.type, dect_mac_event_to_str(msg_to_queue.type));
		err = k_msgq_put(&mac_event_msgq, &msg_to_queue, K_NO_WAIT);
		LOG_DBG("Queued MAC event %d: ", msg_to_queue.type);

		switch (err)
		{
		case 0:
			LOG_DBG("Found Message (%d) ", err);
			break;
		case -42:
			LOG_DBG("NO Message (%d) ", err);
			break;
		case -11:
			LOG_DBG("Try Again (%d) ", err);
			break;			
		default:
			LOG_DBG("(%d)... ", err);
			break;
		...
		if (err != 0) {
			LOG_ERR(" MAC event message queue FULL! Event type %d (PHY ID 0x%02X) was dropped. Error: %d ",
...
{
	int err;
	LOG_DBG("nrf_modem_dect_phy_event_handler_set() Starting...");

	err = nrf_modem_dect_phy_event_handler_set(phy_event_handler_callback);
    if (err) {
        LOG_ERR("Failed to set nRF DECT PHY event handler, err: %d", err);
		return err;
    } else {
        LOG_INF("nRF DECT PHY event handler registered successfully.");
    }
...
{
    LOG_DBG("[OP_HANDLE_MAP_DEBUG] %s - Operation Handle Map State:", test_name);
    for (int i = 0; i < MAX_CONCURRENT_PHY_OPS; i++) {
        if (g_op_handle_map[i].handle != 0) {
            LOG_DBG("  Slot %d: Handle %u, Context %p", 
                   i, g_op_handle_map[i].handle, g_op_handle_map[i].ctx);
        }
    }
    LOG_DBG("[OP_HANDLE_MAP_DEBUG] Total active handles: %d", count_active_handles());
}
...
void dect_mac_phy_if_register_op_handle(uint32_t handle, struct dect_mac_context *ctx)
{
	k_spinlock_key_t key = k_spin_lock(&g_phy_if_lock);
	for (int i = 0; i < MAX_CONCURRENT_PHY_OPS; i++) {
		// LOG_DBG("  - g_op_handle_map[%d].handle:%d ",i ,g_op_handle_map[i].handle);
		if (g_op_handle_map[i].handle == 0 || !g_op_handle_map[i].handle) {
			g_op_handle_map[i].handle = handle;
			g_op_handle_map[i].ctx = ctx;

			LOG_DBG("[PHY_IF_MAP] Registered handle %u to context %p (Role %s) - g_op_handle_map[%d].handle:%d ",
	...
	k_spin_unlock(&g_phy_if_lock, key);
	LOG_DBG("[PHY_IF_MAP] ***ERROR***: No free slots to register handle %u", handle);
}
...
void dect_mac_phy_if_deregister_op_handle(uint32_t handle)
{
	...
	for (int i = 0; i < MAX_CONCURRENT_PHY_OPS; i++) {
		if (g_op_handle_map[i].handle == handle) {
            LOG_DBG("[PHY_IF_MAP] Deregistering handle %u (context %p) from slot %d",
		...
		// LOG_DBG("  - Registered g_op_handle_map[%d].handle:%d ",i ,g_op_handle_map[i].handle);
	}
	k_spin_unlock(&g_phy_if_lock, key);
}
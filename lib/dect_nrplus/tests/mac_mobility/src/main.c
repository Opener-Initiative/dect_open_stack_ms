...
ZTEST(dect_mac_mobility, test_pt_handover)
{
	...
	dect_mac_test_set_active_context(&g_mac_ctx_ft1);
	mock_phy_set_active_context(&g_phy_ctx_ft1);
	// g_mac_ctx_ft1.role_ctx.ft.operating_carrier = 3; /* Set distinct channels */
	...
	dect_mac_test_set_active_context(&g_mac_ctx_ft2);
	mock_phy_set_active_context(&g_phy_ctx_ft2);
	g_mac_ctx_ft2.role_ctx.ft.operating_carrier = 1; /* Set distinct channels */
	nrf_modem_dect_phy_activate(NRF_MODEM_DECT_PHY_RADIO_MODE_LOW_LATENCY);
	dect_mac_start();
	process_all_mac_events();
	zassert_true(run_simulation_until(2000000, is_ft2_beaconing), "FT never started beaconing");
	...
	strong_beacon.pcc_data.hdr.hdr_type_1.transmitter_id_lo = (uint8_t)(g_mac_ctx_ft2.own_short_rd_id & 0xFF);
	strong_beacon.pcc_data.header_status = NRF_MODEM_DECT_PHY_HDR_STATUS_VALID;
	...
	g_last_tx_pdu_len_capture = 0;
	zassert_true(run_simulation_until(100000, ft2_sent_handover_response),
...
ZTEST_SUITE(dect_mac_mobility, 
           NULL,                    /* No suite setup */
           dect_mac_mobility_setup,   /* Suite setup (once) */
           dect_mac_mobility_before,   /* Before each test */
           dect_mac_mobility_after,    /* After each test */

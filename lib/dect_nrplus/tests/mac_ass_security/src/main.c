...
static void set_master_psk(dect_mac_context_t *ctx)
{
    /* Define a known good PSK (16 bytes for AES-128) */
    uint8_t master_psk[16] = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF
    };
    
    /* Copy PSK directly to context's master_psk field */
    memcpy(ctx->master_psk, master_psk, sizeof(master_psk));
    
    printk("[TEST] Master PSK set manually for context %p: ", (void *)ctx);
    for (int i = 0; i < 8; i++) {
        printk("%02X", master_psk[i]);
    }
    printk("...\n");
    
    /* Note: security_build_iv doesn't return a value (void function) */
    uint8_t test_iv[16];
    security_build_iv(test_iv, ctx->own_long_rd_id, 0xAABBCCDD, 1, 1);
    printk("[TEST] IV built successfully (no return value to check)\n");
}

static void debug_mac_psk(dect_mac_context_t *ctx, const char *name)
{
    printk("\n=== MAC PSK DEBUG for %s ===\n", name);
    printk("Context address: %p\n", (void *)ctx);
    
    /* Print the master_psk field (assuming it's a fixed-size array) */
    printk("master_psk (first 16 bytes): ");
    for (int i = 0; i < 16; i++) {
        printk("%02X", ctx->master_psk[i]);
    }
    printk("\n");
    ...
    int all_zero = 1;
    for (int i = 0; i < 16; i++) {
        if (ctx->master_psk[i] != 0) {
            all_zero = 0;
            break;
        }
    }
    printk("PSK is %s\n", all_zero ? "ALL ZEROS (uninitialized!)" : "non-zero (looks valid)");
    
    /* Try a direct crypto operation with this key */
    if (!all_zero) {
        uint8_t test_mic[5];
        uint8_t test_data[16] = {0};
        
        int ret = security_calculate_mic(test_data, sizeof(test_data), 
                                         ctx->master_psk, test_mic);
        printk("Direct MIC calc with context PSK: %s (ret=%d)\n",
               ret == 0 ? "SUCCESS" : "FAILED", ret);
    }
    
    printk("=== END PSK DEBUG ===\n");
}

static void debug_ctr_availability(void)
{
    printk("\n=== CTR DEBUG ===\n");
    ...
    /* Test CTR encryption availability */
    ...
    status = psa_import_key(&attributes, test_key, sizeof(test_key), &key_id);
    printk("CTR key import: %d (%s)\n", (int)status,
           status == PSA_SUCCESS ? "SUCCESS" : "FAILED");
    
    if (status == PSA_SUCCESS) {
        /* Try a test encryption */
        // status = psa_cipher_encrypt(key_id, PSA_ALG_CTR, test_iv, sizeof(test_iv),
        //                            test_data, sizeof(test_data), 
        //                            test_data, sizeof(test_data), &output_len);
        printk("CTR test encryption: %d\n", (int)status);
    ...
    /* Check Kconfig */
    printk("\nKconfig status:\n");
    printk("  PSA_WANT_ALG_CTR: %s\n", 
           IS_ENABLED(CONFIG_PSA_WANT_ALG_CTR) ? "enabled" : "DISABLED");
    printk("  MBEDTLS_CIPHER_MODE_CTR: %s\n", 
           IS_ENABLED(CONFIG_MBEDTLS_CIPHER_MODE_CTR) ? "enabled" : "DISABLED");
    
    printk("=== END CTR DEBUG ===\n");
}

static void debug_crypto_status(void)
{
    psa_status_t status;
    psa_key_id_t key_id = 0;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    ...
    /* Try a simple key operation to verify functionality */
    psa_set_key_lifetime(&attributes, PSA_KEY_LIFETIME_VOLATILE);
    psa_set_key_type(&attributes, PSA_KEY_TYPE_AES);
    psa_set_key_bits(&attributes, 128);
    ...
    /* Test key import with a known key */
    uint8_t test_key[16] = {0};
    status = psa_import_key(&attributes, test_key, sizeof(test_key), &key_id);
    printk("psa_import_key test: %d\n", (int)status);
    
    if (status == PSA_SUCCESS) {
        psa_destroy_key(key_id);
    }
    
    printk("--- END CRYPTO DEBUG ---\n");
}

/* --- Enhanced Test Setup --- */

static void *dect_mac_ass_secure_setup(void)
{
	psa_status_t status = psa_crypto_init();
    printk("dect_mac_ass_secure_setup psa_crypto_init %d \n", status);
...
static void dect_mac_ass_secure_before(void *fixture)
{
    ARG_UNUSED(fixture);
    int err;

    /* Initialize PSA Crypto - THIS IS CRITICAL */
    psa_status_t psa_status = psa_crypto_init();
    if (psa_status != PSA_SUCCESS) {
        printk("ERROR: psa_crypto_init() failed with status: %d\n", (int)psa_status);
        /* Continue anyway - the test will fail with useful error messages */
    } else {
        printk("PSA Crypto initialized successfully\n");
    }
    /* verify if CTR is available */
    debug_ctr_availability();
    ...
    /* MANUALLY SET THE PSK FOR FT */
    set_master_psk(&g_mac_ctx_ft);    
    /* Check crypto status after FT init */
    debug_crypto_status();
    ...
    /* MANUALLY SET THE PSK FOR PT */
    set_master_psk(&g_mac_ctx_pt);    
    /* Check crypto status after PT init */
    debug_crypto_status();
    ...
    /* After MAC core init */
    debug_mac_psk(&g_mac_ctx_ft, "FT");
    debug_mac_psk(&g_mac_ctx_pt, "PT");
...
static void dect_mac_ass_secure_after(void *fixture)
{
    ARG_UNUSED(fixture);
...
/* --- Test Cases --- */
ZTEST(dect_mac_ass_secure, test_pt_ft_association)
{
    printk("\n--- RUNNING TEST: %s ---\n", __func__);
...
ZTEST_SUITE(dect_mac_ass_secure, 
           NULL,                    /* No suite setup */
           dect_mac_ass_secure_setup,   /* Suite setup (once) */
           dect_mac_ass_secure_before,   /* Before each test */
           dect_mac_ass_secure_after,    /* After each test */

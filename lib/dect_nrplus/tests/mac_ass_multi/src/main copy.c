...
/* This test works but the number of nodes is limited to 13 (3FT + 10PT) due to buffers and memory*/
#define NUM_FTS 3
#define NUM_PTS 4
...
// 	/* 3. Start PTs in a STAGGERED manner so they associate with different FTs */
//     printk("\n\n[TEST] 3. Staggered starting of %d PTs\n", NUM_PTS);
    
//     // Stagger PT starts with increasing delays
//     int count = 0;
//     for (int i=0; i<(NUM_PTS / 2); i++) {
//         // Calculate staggered start time - each PT starts after a delay
//         // This gives earlier PTs time to associate before later ones start
//         uint32_t stagger_delay_us = 200; // stagger between PTs
//         printk("  - Scheduling PT[%d] to start after %d us\n", i, stagger_delay_us);
        
//         // Run simulation until it's time to start this PT
//         run_simulation_until(stagger_delay_us, NULL);
        
//         // Start the PT
//         printk("  - Starting PT[%d] now at simulation time: %llu us\n", 
//                i, stagger_delay_us);
...
//         // Process events immediately after starting this PT
//         process_all_mac_events();
        
//         // Optional: Let this PT run for a while before starting the next one
//         if (i < NUM_PTS - 1) {
//             printk("  - Letting PT[%d] run for before starting next PT\n", i);
//             run_simulation_until(100, NULL);
//         }
//         count++;
//     }
    
// 	// After all PTs are started, run simulation until they're all associated
//     printk("\n\n[TEST] 4. Running simulation until all PTs are associated\n");
// 	process_all_mac_events();
// 	zassert_true(run_simulation_until(50000000, are_all_pts_associated), "Not all PTs became associated");

//     for (int j=count; j<NUM_PTS; j++) {
//         // Calculate staggered start time - each PT starts after a delay
//         // This gives earlier PTs time to associate before later ones start
//         uint32_t stagger_delay_us = 200; // stagger between PTs
//         printk("  - Scheduling PT[%d] to start after %d us\n", j, stagger_delay_us);
...
//         dect_mac_test_set_active_context(&g_mac_ctx_pt[j]);
//         mock_phy_set_active_context(&g_phy_ctx_pt[j]);
//         nrf_modem_dect_phy_activate(NRF_MODEM_DECT_PHY_RADIO_MODE_LOW_LATENCY);
//         dect_mac_start();
...
//     printk("\n\n[TEST] 5. Final assertions and Association Report\n");
//     for (int i=0; i<NUM_PTS; i++) {
...
ZTEST(dect_mac_assoc, test_pt_ft_association)
{
	...
	process_all_mac_events();
	zassert_true(run_simulation_until(500000, are_all_pts_associated), "Not all PTs became associated");
